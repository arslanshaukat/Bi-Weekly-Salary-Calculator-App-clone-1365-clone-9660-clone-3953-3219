...
const Header = () => {
  ...
  const navLinks = [
    { path: '/', label: 'Personnel', icon: FiUsers },
    { path: '/attendance', label: 'Attendance', icon: FiCalendar },
    { path: '/calculate', label: 'Process', icon: FiCalculator },
    { path: '/results', label: 'Archives', icon: FiArchive },
    { path: '/summary', label: 'Summary', icon: FiPieChart },
  ];
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed top-0 left-0 bottom-0 w-[80%] max-w-sm bg-white z-[70] shadow-2xl lg:hidden flex flex-col">
              ...
              <div className="flex-grow py-6 overflow-y-auto">
                <div className="px-6 mb-8 text-left">
                  ...
                  <div className="space-y-2">
                    {navLinks.map((link) => (
                      <Link key={link.path} to={link.path} className={`flex items-center space-x-4 p-4 rounded-2xl transition-all ${isActive(link.path) ? 'bg-blue-600 text-white shadow-xl shadow-blue-100' : 'text-gray-600 hover:bg-gray-50 font-bold'}`} >
